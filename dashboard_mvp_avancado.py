...
st.markdown(
    ...
    "Monitoramento Inteligente da Produção Leiteira · IA preditiva · Clima · IoT-ready"
    "</p>",
...
st.markdown(
    """
**Navegação** - Use o menu na barra lateral esquerda para abrir cada área:

| Seção | Conteúdo |
|--------|----------|
| **Executivo** | IA preditiva para risco, variáveis críticas e proteção da cadeia leiteira |
| **Operacional** | Rotas, tanque, custo por litro e redução de perdas |
| **Produtores** | Qualidade do leite, risco e adequação para derivados premium |
...
| **Fornecedores 360** | Risco, qualidade, continuidade e leitura de produção por fazenda |
...
Os **filtros** (laticínio, polo e período) aplicam-se às páginas analíticas; em **Gestão e Dados** também alimentam o comparativo.
"""
)

if not ctx.pred_f.empty:
    st.subheader("Leitura executiva - média diária filtrada")
    serie = ctx.pred_f.groupby("data", as_index=False).agg(
        y_real=("y_real", "mean"),
        y_pred=("y_pred_modelo", "mean"),
    )
    fig = px.line(serie, x="data", y=["y_real", "y_pred"], title="Realizado × previsto (filtros atuais)")
...
st.caption(
    "O VIA LEITE SENSE transforma dados da fazenda em inteligência operacional para elevar a qualidade do leite destinado a derivados premium."
)
st.caption("Comando: `streamlit run dashboard_mvp_avancado.py`")
